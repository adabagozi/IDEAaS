...
public class DistanceComputerParallelJavaFeatures  implements IDistanceComputer {
...
//		System.out.println("DistanceComputerParallelJavaFeatures -> foundPositionsNearestKernels");
		return Stream.of(kernels).map(k -> {
			double distance = VectorsUtility.distance(instance.toDoubleArray(), k.getCenter(), CluStreamAlgorithmImplementation.JAVA_PARALLEL_FEATURES);
...
//		System.out.println("DistanceComputerParallelJavaFeatures -> computeMinDistanceKernels");
		 return Stream.of(kernels).mapToDouble(k -> {
			 if(kernel.getPosition() == k.getPosition()) {
			  ...
			  return VectorsUtility.distance(k.getCenter(), kernel.getCenter(), CluStreamAlgorithmImplementation.JAVA_PARALLEL_FEATURES);
		}).min().getAsDouble();
	}
...
//		System.out.println("DistanceComputerParallelJavaFeatures -> foundPositionsNearestKernels");
		...
		for ( int i = 0; i < kernels.length; i++ ) {
			...
			NearesKernelsPositionsInArray nearestKernel2 = IntStream
					...
					.filter(j -> j != i2)
					.mapToObj(j -> {
						return new NearesKernelsPositionsInArray(i2, j, VectorsUtility.distance(centerA, kernels[j].getCenter(), CluStreamAlgorithmImplementation.JAVA_PARALLEL_FEATURES));
					})
