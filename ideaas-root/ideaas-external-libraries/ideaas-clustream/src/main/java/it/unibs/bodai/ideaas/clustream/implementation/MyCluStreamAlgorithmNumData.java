...
public class MyCluStreamAlgorithmNumData extends MyAbstractClusterer implements ICluStreamAlgorithm {
	...
	public MyCluStreamAlgorithmNumData() {
		this.iDistanceComputer = DistanceComputerFactory.getDAOFactory().getDistanceComputer();
	}
	
	public MyCluStreamAlgorithmNumData(String distanceIdentificator) {
...
//		System.out.println("Based on density and number of date lower than 1%");
...
//				return; e' necessario togliere return per evitare di perdere il dato di confine
	...
	private void removeOldKernel(Instance instance, MyClustreamKernel[] kernels) {
		...
		int totalData = this.getTotalData(kernels);
...
//					return;
//				}else {
					if(kernels[i].getN() > (totalData*StaticVariables.PERCENT_DATA_THRESHOLD)) {
						double densityTemp = kernels[i].getDensity();
						if (density < densityTemp) { // density > densityTemp
							oldestIndex = i;
							density = densityTemp;
						}
					}
//				}
		...
		if (oldestIndex>=0) {
//			System.out.println("Remove based on density and number of date lower than 1%");
			kernels[oldestIndex] = new MyClustreamKernel( instance, instance.numValues(), timestamp, t, m, this.getMaxID(), oldestIndex);

		}
	}
	
	private int getTotalData(MyClustreamKernel[] kernels) {
		int totalData = 0;

		for ( int i = 0; i < kernels.length; i++ ) {
			totalData = (int) (totalData + kernels[i].getN());
		}
