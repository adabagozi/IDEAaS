...
public class MyParallelSparkCluStreamAlgorithmBuffer extends MyAbstractClusterer implements ICluStreamAlgorithm {
...
//	private List<MyClustreamKernel> buffer; // Buffer for initialization with kNN
	...
	public MyParallelSparkCluStreamAlgorithmBuffer(CluStreamAlgorithmImplementation algorithm) throws Exception {
//		this.distanceComputer = new DistanceComputer();
		this.iDistanceComputer = DistanceComputerFactory.getDAOFactory().getDistanceComputer(algorithm);
//		resetLearningImpl();
	}
	
	public MyParallelSparkCluStreamAlgorithmBuffer(CluStreamAlgorithmImplementation algorithm, SparkSession sparkSession) throws Exception {
//		this.distanceComputer = new DistanceComputer();
...
//		ReadOnlyArrayList<Instance> readOnlyKernels = new ReadOnlyArrayList<>(instances.);
...
//			indexes.add(k);
//		});
		Dataset<Long> indexes = sparkSession.range(0, instances.numInstances()-1);
		
		indexes.foreach(k ->{
//			System.out.println("i= "+i);
			int ind = Integer.parseInt(""+k);
			Instance inst = instances.get(ind);
			// 1. Determine closest kernel
			NearestKernel nearestKernel = iDistanceComputer.foundNearestKernel(instances.get(ind), kernels);
...
//			// 1. Determine closest kernel
//			NearestKernel nearestKernel = distanceComputer.foundNearestKernel(instances.get(k), kernels);
//			
//			// 2. Determine if the point belongs to his nearest kernel
//			double computeRadius = this.computeRadius(nearestKernel.getNearestKernel(), kernels);
//
...
//	}
	...
	private double computeRadius(MyClustreamKernel kernel, MyClustreamKernel[] kernels) {
		...
		if ( kernel.getWeight() == 1 ) {
			...
			radius = distanceComputer.computeMinDistanceKernels(kernel, kernels);
...
//			MyClustreamKernel k = kernels.get(kernelAge.getOldestKernel().getPosition());
	...
	private void mergeClosestKernels(Instance instance, MyClustreamKernel[] kernels) {
		...
		NearesKernelsPositionsInArray nearestPositions = distanceComputer.foundPositionsNearestKernels(kernels);
