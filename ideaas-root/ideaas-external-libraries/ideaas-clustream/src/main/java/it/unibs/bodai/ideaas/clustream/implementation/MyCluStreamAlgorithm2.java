...
public class MyCluStreamAlgorithm2 extends MyAbstractClusterer implements ICluStreamAlgorithm {
	...
	public MyCluStreamAlgorithm2() {
		this.iDistanceComputer = DistanceComputerFactory.getDAOFactory().getDistanceComputer();
	}
	
	public MyCluStreamAlgorithm2(String distanceIdentificator) {
...
//				System.out.println( mCK.getParentHistoryId().toString());
				oldKernelsHistoryId = mCK.getParentHistoryId();
				if(oldKernelsHistoryId.isEmpty()) {
					oldKernelsHistoryId = oldKernelsId;
				}
			}
		}else {
			for (MyClustreamKernel mCK: oldKernels) {
...
//		int dim = instance.numValues();
		...
		if (!initialized) {
			if (buffer.size() < bufferSize) {
				buffer.add(new MyClustreamKernel(instance, instance.numValues(), timestamp, t, m, -1, buffer.size()));
				return;
			} else {
...
//				return; e' necessario togliere return per evitare di perdere il dato di confine
	...
	private void mergeClosestKernels(Instance instance, MyClustreamKernel[] kernels) {
		...
		//TODO Rendere gli id cluster precedenti cumulativi.
		...
		kernels[nearestPositions.getClosestB()] = new MyClustreamKernel( instance, instance.numValues(), timestamp, t,  m, this.getMaxID(), nearestPositions.getClosestB());
	}
