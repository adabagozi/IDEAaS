...
public class MyClustering extends AbstractMOAObject{

    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private AutoExpandVector<MyCluster> clusters;

    public MyClustering() {
        this.clusters = new AutoExpandVector<MyCluster>();
    }

    public MyClustering( MyCluster[] clusters ) {
//    		System.out.println(this.getClass().getName() + " -> clusters.length: "+clusters.length);
        this.clusters = new AutoExpandVector<MyCluster>();
        for (int i = 0; i < clusters.length; i++) {
//        		MyCluster temp = clusters[i];
//        		temp.addKernel(temp);
            this.clusters.add(clusters[i]);
        }
    }

    public MyClustering(List<? extends Instance> points){
        ...
        for (int i = 0; i < numClasses; i++) {
            if(sorted_points[i].size()>0){
            		MySphereCluster s = new MySphereCluster(sorted_points[i],dim);
    ...
    public MyClustering(ArrayList<DataPoint> points, double overlapThreshold, int initMinPoints){
        HashMap<Integer, Integer> labelMap = MyClustering.classValues(points);
        int dim = points.get(0).dataset().numAttributes()-1;
        ...
        clusters = new AutoExpandVector<MyCluster>();
        int microID = 0;
        for (int i = 0; i < numClasses; i++) {
            ArrayList<MySphereCluster> microByClass = new ArrayList<MySphereCluster>();
            ...
            while(pointInCluster.size()>0){
                ...
                if(micro_points.size() > 0){
                		MySphereCluster s = new MySphereCluster(micro_points,dim);
                    for (int c = 0; c < microByClass.size(); c++) {
                        if(((MySphereCluster)microByClass.get(c)).overlapRadiusDegree(s) > overlapThreshold ){
                            micro_points.addAll(pointInMicroClusters.get(c));
                            s = new MySphereCluster(micro_points,dim);
...
//                        System.out.println("Overlap C"+(clustering.size()+c)+" ->C"+(clustering.size()+c1)+": "+overlap);
                        if(overlap > overlapThreshold){
                            ...
                            MySphereCluster s = new MySphereCluster(pointInMicroClusters.get(c),dim);
    ...
    public MyClustering(AutoExpandVector<MyCluster> clusters) {
     ...
     */
    public void add(MyCluster cluster){
     ...
     * remove a cluster from the clustering
     */
    public MyCluster get(int index){
        if(index < clusters.size()){
     ...
     */
//    public AutoExpandVector<MyCluster> getClustering() {
//        return clusters;
//    }
//
     ...
     */
    public AutoExpandVector<MyCluster> getClusteringCopy() {
        return (AutoExpandVector<MyCluster>)clusters.copy();
    }
