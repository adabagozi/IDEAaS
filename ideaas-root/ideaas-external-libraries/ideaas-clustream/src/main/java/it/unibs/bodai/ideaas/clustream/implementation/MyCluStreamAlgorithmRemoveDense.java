...
public class MyCluStreamAlgorithmRemoveDense extends MyAbstractClusterer implements ICluStreamAlgorithm {
	...
	public MyCluStreamAlgorithmRemoveDense() {
		this.iDistanceComputer = DistanceComputerFactory.getDAOFactory().getDistanceComputer();
	}
	
	public MyCluStreamAlgorithmRemoveDense(String distanceIdentificator) {
...
//		System.out.println("Based on density");
...
//				System.out.println("Treshold: "+threshold+"     kernels[i].getRelevanceStamp(): "+kernels[i].getRelevanceStamp() + "     timestamp:"+timestamp);
//				if(kernels[i].getN()==1) {
//					oldestIndex = i;
//					return;
//				}else {
					double densityTemp = kernels[i].getDensity();
					if (density < densityTemp) {
						oldestIndex = i;
						density = densityTemp;
					}
//				}
