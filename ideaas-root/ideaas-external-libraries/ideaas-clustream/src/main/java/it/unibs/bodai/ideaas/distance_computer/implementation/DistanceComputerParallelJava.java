...
public class DistanceComputerParallelJava  implements IDistanceComputer {
	
	@Override
	public NearestKernel foundNearestKernel(Instance instance, MyClustreamKernel[] kernels) {
//		System.out.println("DistanceComputerParallelJava -> foundNearestKernel");
		return Stream.of(kernels).parallel().map(k -> {
			double distance = VectorsUtility.distance(instance.toDoubleArray(), k.getCenter(), CluStreamAlgorithmImplementation.JAVA_PARALLEL_BOTH);
...
//		System.out.println("DistanceComputerParallelJava -> computeMinDistanceKernels");
		 return Stream.of(kernels).parallel().mapToDouble(k -> {
			 if(kernel.getPosition() == k.getPosition()) {
				 return Double.MAX_VALUE;
			 }
			
			  return VectorsUtility.distance(k.getCenter(), kernel.getCenter(), CluStreamAlgorithmImplementation.JAVA_PARALLEL_BOTH);
		}).min().getAsDouble();
	}
	
	@Override
	public NearesKernelsPositionsInArray foundPositionsNearestKernels(MyClustreamKernel[] kernels) {
//		System.out.println("DistanceComputerParallelJava -> foundPositionsNearestKernels");
		...
		for ( int i = 0; i < kernels.length; i++ ) {
			...
			NearesKernelsPositionsInArray nearestKernel2 = IntStream
					...
					.filter(j -> j != i2)
					.mapToObj(j -> {
						return new NearesKernelsPositionsInArray(i2, j, VectorsUtility.distance(centerA, kernels[j].getCenter(), CluStreamAlgorithmImplementation.JAVA_PARALLEL_BOTH));
					})
