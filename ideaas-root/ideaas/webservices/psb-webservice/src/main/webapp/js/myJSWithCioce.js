...
function readDataResult(result){
	...
	var cioceSynthesis = result['cioce_synthesis'];
	
	var keys = ["X", "Y"];

	if(ideaasSynthesis.length == cioceSynthesis.length ){

		console.log("numero sintesi ok");
		...
		for (var i = 0; i < ideaasSynthesis.length; i++) {
			...
			addElementsResultData(i, sintesiIdeaas["timestamp_initial"], sintesiCioce["timestamp_initial"]);
			...
			queue.push({sintesiIdeaas: buildData(sintesiIdeaas["synthesis"]), sintesiCioce: buildData(sintesiCioce["synthesis"]), clusterIdeaas: buildDataCluster(sintesiIdeaas["clusters"]), clusterCioce: buildDataCluster(sintesiCioce["clusters"])});
		}
		plotAsync(queue, 0, keys);
	}else {
		console.log("sintesi diverso numero: "+ ideaasSynthesis.length + " - " + cioceSynthesis.length);
	}
}
...
function plotAsync(queue, id, keys){
	if(queue.length>0){
		setTimeout(function() {
			...
			// data.sintesiIdeaas 
			plotGraph("graph"+id, data.sintesiIdeaas, data.sintesiCioce, data.clusterIdeaas, data.clusterCioce, keys);	
			$('#loader'+id).hide();
...
function addElementsResultData(id, dateIdeaas, dateCioce){
	...
	$('#resultData').append('<div class="row" style="height: 450px;"><div class="col-xs-12"> <div class="col-lg-4 col-md-5 col-sm-12" id='+ resultRowID +'>'+
								'<div class="col-xs-12" id='+ resultRowIdeaasID +'><p><strong>Sintesi Nuove: ' + dateIdeaas + '</strong></p><br></div>'+
								'<div class="col-xs-12" id='+ resultRowCioceID +'><br><br><p><strong>Sintesi Cioce: ' + dateCioce + '</strong></p><br></div>'+'</div>'+
...
function plotGraph(id, data1, data2, clusters1, clusters2, keys){
	Highcharts.chart(id, {
		chart : {
			type : 'bubble',
			plotBorderWidth : 1,
